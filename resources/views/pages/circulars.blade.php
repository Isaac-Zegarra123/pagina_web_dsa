    ...
    <div class=" service pb-5">
        <div class="col-lg-12">
            <div class="resolutions-section-title d-flex align-items-center justify-content-center">
                ...
                <!-- Título centrado -->
                <h2 class="sub-t mb-0 flex-grow-1 text-center">Circulares, Notas de Instruccion y comunicados</h2>
            </div>
        </div>
            ...
            <div class="year-selector">
                ...
                <select id="yearDropdown"  onchange="location = this.value;">
                    @foreach($years as $year)
                        <option value="{{ route('pages.circulars', $year) }}" {{ $currentYear == $year ? 'selected' : '' }}>
                            {{ $year }}
                        </option>
                    @endforeach
                </select>
        ...
        @include('partials.circular', ['circulares' => $circulares])
        </div>
        </div>
    </div>
      ...
      $('.filter-year').on('click', function() {
          ...
          $.ajax({
              ...
              success: function(response) {
                  ...
                  $('#circulares-container').html(response);
              },
              error: function() {
