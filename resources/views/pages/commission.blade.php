...
<body>
  ...
  <div class=" service pb-5">
    <div class="row">
        <div class="col-lg-12">
            <div class="resolutions-section-title d-flex align-items-center justify-content-center">
                ...
                <!-- Título centrado -->
                <h2 class="sub-t mb-0 flex-grow-1 text-center">Dictámenes de Consejo Universitario</h2>
            </div>
        </div>
            ...
            <div class="year-selector">
                ...
                <select id="yearDropdown"  onchange="location = this.value;">
                    @foreach($years as $year)
                        <option value="{{ route('pages.commission', $year) }}" {{ $currentYear == $year ? 'selected' : '' }}>
                            {{ $year }}
                        </option>
                    @endforeach
                </select>
    ...
    <div class="row curricular-menu-curricular-container" id="dictamenes-container">
        @include('partials.dictamen', ['dictemen' => $dictamenes])
    </div>
      </div>
  </div>
      ...
      $('.filter-year').on('click', function() {
          ...
          $.ajax({
              ...
              success: function(response) {
                  ...
                  $('#dictamenes-container').html(response);
              },
              error: function() {
